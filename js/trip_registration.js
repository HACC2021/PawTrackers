$(function() {
    ...
    $('#program_dropdown').change(function() {
        ...
        const program = new Program(programs[program_id]);
        const price = program.base_price
    ...
    $('#airline_dropdown').change(function() {
        ...
        const flight_objs = flights[airline_id];
        for (const [flight_no, _] of Object.entries(flight_objs)) {
            $('#flight_no_dropdown').append($('<option>', {
                text: flight_no
            }));
        }
    });
...
$(document).ready(function() {
    $("#submit_trip").click(function() {
        ...
        const flight_no = $('#flight_no_dropdown').children('option:selected').val()
        ...
        const flight = new Flight(JSON.parse(sessionStorage.flights)[airline][flight_no])
